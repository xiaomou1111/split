...
- `policy` 判定顺序固定（policy.h 注释即真相）：skip_uid → v6 不分类直连（ipv6_classify=false）→ 内置本地段 → proxy → direct → CNIP（仅 cnip=on）→ default。
- ctl 协议：Unix socket **单命令一连接**，回复后即 close；命令前缀 `stats/status/list-rules/reload/reload-cnip/update-cnip/cnip/add-rule/del-rule/stop`。
