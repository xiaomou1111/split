...
- `splitctl`（cli/）：`start stop status stats list-rules reload reload-cnip update-cnip cnip add-rule del-rule validate`，经 Unix socket 与 daemon 通信。
...
- **policy 判定顺序固定（7 步，policy.h 注释即真相）**：skip_uid → v6 且 ipv6_classify=0 → 内置本地段 → proxy → direct → CNIP（仅 `cnip=on`）→ default。改顺序 = 改分流语义，必须同步 docs/01、docs/02、README。
- **ctl 协议**：Unix socket，**单命令一连接**（回复后即 close，勿改回多命令循环）；命令前缀 `stats/status/list-rules/reload/reload-cnip/update-cnip/cnip/add-rule/del-rule/stop`；回复首行 `OK/ERR`、末行 `END`。`status` 字段与 WARN 行被 WebUI app.js 解析，改格式必须同步 app.js。
